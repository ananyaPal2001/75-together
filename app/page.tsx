...
export default function Home() {
  ...
  if (view === "summary") return <DaySummary challenge={challenge} members={members} session={session} day={day} onProfileChange={refreshMembers} onSignOut={signOut} onNewChallenge={() => { setChallenge(null); navigate("setup"); }} onBack={() => navigate("dashboard", true)} />;
  if (view === "journal") return <Journal challenge={challenge} members={members} userId={session?.user.id ?? ""} session={session} onProfileChange={refreshMembers} onSignOut={signOut} onNewChallenge={() => { setChallenge(null); navigate("setup"); }} registerSave={(save) => { saveBeforeLeave.current = save; }} day={day} onBack={() => navigate("dashboard", true)} />;
